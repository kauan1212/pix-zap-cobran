...
const BillingManager = ({ clients, onDataChange }: BillingManagerProps) => {
  ...
  const handleReceiptUploaded = async (billingId: string, url: string) => {
    try {
      ...
      toast({
        title: "Sucesso",
        description: "Comprovante carregado com sucesso! Aguardando confirmação do administrador.",
      });
      ...
      toast({
        title: "Erro",
        description: "Erro ao salvar o comprovante.",
        variant: "destructive",
      });
    }
  };
  ...
  const updateBillingStatus = async (billingId: string, status: Billing['status']) => {
    ...
    if (status === 'paid') {
      ...
      updateData.receipt_confirmed_at = new Date().toISOString();
    }
...
Após o pagamento, envie o comprovante para confirmarmos.`;
  ...
  const generateOverdueMessage = (billing: Billing) => {
    ...
    if (daysOverdue > 0) {
      finalAmount = billing.amount * 1.1;
      
      if (daysOverdue > 0) {
        const dailyInterestRate = 0.0004;
        ...
        <CardHeader>
          <div className="flex flex-col space-y-4">
            ...
            <div className="flex gap-4">
              <Button
                variant={activeTab === 'billings' ? 'default' : 'outline'}
          ...
          {activeTab === 'billings' && (
            <>
              ...
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h3 className="text-lg font-semibold">Cobranças por Cliente</h3>
                  <p className="text-muted-foreground">Selecione um cliente para visualizar as cobranças</p>
                </div>
                              ...
                              <CardContent>
                                ...
                                {billing.receipt_url ? (
                                  ...
                                  <ReceiptUpload billingId={billing.id} onUploaded={url => {
                                    handleReceiptUploaded(billing.id, url);
                                  }} />
                                )}
                              </CardContent>
                            </Card>
